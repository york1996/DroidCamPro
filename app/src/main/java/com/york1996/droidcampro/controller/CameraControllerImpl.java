...
public class CameraControllerImpl {
    ...
    public CameraControllerImpl(Context context, AutoFitTextureView previewTextureView) {
        mContext = context;
    ...
    private boolean isHardwareLevelSupported(CameraCharacteristics c, int requiredLevel) {
        ...
        Log.d(TAG, "deviceLevel = " + deviceLevel);
    ...
    private void openCameraAndPreview(int width, int height, boolean front) {
        try {
            for (String cameraId : mCameraManager.getCameraIdList()) {
                CameraCharacteristics characteristics = mCameraManager.getCameraCharacteristics(cameraId);
                isHardwareLevelSupported(characteristics, 1);
                Integer facing = characteristics.get(CameraCharacteristics.LENS_FACING);
